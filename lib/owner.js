...
// Trim both sides: a stray space or newline from pasting (or in the stored secret) should not lock the owner out.
function codeMatches(code){const secret=ownerCode().trim();if(!secret||typeof code!=='string'||!code.trim())return false;code=code.trim();return crypto.timingSafeEqual(crypto.createHash('sha256').update(code).digest(),crypto.createHash('sha256').update(secret).digest());}
