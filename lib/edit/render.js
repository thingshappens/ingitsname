...
async function generate(order){
  // A character recording source is deliberately required before generation
  // can return. The former ElevenLabs request was removed rather than used as
  // an invisible fallback while Felix's approved source takes shape.
  void order;
  throw new Error('HSC character recordings are being prepared.');
}
