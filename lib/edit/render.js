...
function filters(c,order,samples){
  ...
  if(c.style==='chopped_up'){
    ...
    const delay=60000/order.bpm;
    style=[`asplit=3[lead][echo][room];[echo]lowpass=f=4200,aecho=0.8:0.24:${delay}:0.20[echoout];[room]aecho=0.8:0.55:38|76|126|186:0.18|0.12|0.08|0.04[roomout];[lead][echoout][roomout]amix=inputs=3:normalize=0`];
  }
...
async function render(pcm,cut,order){
  ...
  if(cut.style==='chopped_up')samples=gate(crush(robot(samples,order.bpm)),order.bpm,cut.groove,cut.cutAmount);
