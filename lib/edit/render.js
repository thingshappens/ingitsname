...
async function generate(order){
  ...
  const response=await fetch(`https://api.elevenlabs.io/v1/text-to-speech/${encodeURIComponent(order.voiceId)}?output_format=pcm_48000`,{method:'POST',headers:{'xi-api-key':process.env.ELEVENLABS_API_KEY,'content-type':'application/json'},body:JSON.stringify({text,model_id:'eleven_v3',voice_settings:{stability:.95,similarity_boost:.85,style:0,use_speaker_boost:true}}),signal:AbortSignal.timeout(60000)});
