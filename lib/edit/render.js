...
function gate(samples,bpm,groove,amount){
  ...
  // The Edit promise is a real, readable gate: half-cycle phrases or tight
  // percussive cuts. A minute bed prevents a digital click without filling the chop.
...
async function generate(order){
  const text=order.phrase;
  const response=await fetch(`https://api.elevenlabs.io/v1/text-to-speech/${encodeURIComponent(order.voiceId)}?output_format=pcm_48000`,{method:'POST',headers:{'xi-api-key':process.env.ELEVENLABS_API_KEY,'content-type':'application/json'},body:JSON.stringify({text,model_id:'eleven_v3',voice_settings:{stability:.82,similarity_boost:.75,style:0,use_speaker_boost:true}}),signal:AbortSignal.timeout(60000)});
