...
function robot(samples,bpm){
  // A restrained, tempo-locked stutter gives the preset its machine movement
  // without making the phrase unintelligible.
  const beat=Math.max(128,Math.round(SR*60/bpm/4)),slice=Math.floor(beat/4);
  return Float32Array.from(samples,(v,i)=>{
    const block=Math.floor(i/beat),local=i%beat;
    if(block%2!==1)return v;
    const edge=local%slice,idx=Math.min(samples.length-1,block*beat+edge);
    return .58*v+.42*samples[idx]*Math.min(1,edge/72);
  });
}
...
function filters(c,order,samples){
  ...
  if(c.style==='sexy_robot'){
    const pitch={low:1,mid:2,bright:3}[order.voiceRange],rate=2**(pitch/12);
    style=[`asetrate=${SR*rate}`,`aresample=${SR}`,`atempo=${1/rate}`,'acrusher=bits=8:mix=0.42','aphaser=in_gain=0.55:out_gain=0.7:delay=3:decay=0.55:speed=0.7:type=t','lowpass=f=9800'];
  }
  const duration=samples.length/SR+(c.style==='clean'?.071:c.style==='dark_echo'?60/order.bpm*2.25:0);
