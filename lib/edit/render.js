...
function gate(samples,bpm,groove,amount){
  // Atelier's pulse shape, arranged as one, one-and-a-half or two musical
  // pulses per beat. It gives a rhythmic chop without blanking spoken words.
  const cycle=SR*60/bpm/({straight:1,triplet:1.5,double:2}[groove]);
  const control=amount==='half'?.68:.90;
  const depth=Math.pow(control,.72),sharpness=Math.pow(control,1.35);
