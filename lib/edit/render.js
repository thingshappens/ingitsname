...
function gate(samples,bpm,groove,amount){
  // A rhythmic volume carve retains a quiet vocal bed between accents. This feels
  // like a produced chop; zeroing the gaps merely made half the phrase disappear.
  const cycle=SR*60/bpm/({straight:1,triplet:1.5,double:2}[groove]);
  const on=cycle*(amount==='half'?.62:.38),curve=amount==='half'?.7:.9;
  // Keep only a ghost of the source between pulses: enough continuity to avoid
  // a dead mute, but deep enough for the chop to read over a beat.
  const bed=amount==='half'?.075:.045;
  return Float32Array.from(samples,(v,i)=>{
    const phase=i%cycle;
    if(phase>=on)return v*bed;
    const accent=Math.pow(Math.sin(Math.PI*phase/on),curve);
    return v*(bed+(1-bed)*accent);
  });
}
function robot(samples,bpm){
  ...
  return Float32Array.from(samples,(v,i)=>{
    ...
    // Keep a substantial signal under each repeat so consonants remain legible.
    const gain=(edge<fade?edge/fade:1)*(local>beat*(1-glitchAmount*.18)?.30:1);
...
function pulse(samples,bpm,amount=78){
  ...
  const cycle=SR*60/bpm/4;
  return Float32Array.from(samples,(v,i)=>{
    const phase=(i%cycle)/cycle;
    const wave=Math.pow(.5-.5*Math.cos(2*Math.PI*phase),1+sharpness*4);
    return v*(1-depth+depth*wave);
  });
}
...
function filters(c,order,samples){
  let style=[];
  if(c.style==='clean')style=['aecho=0.8:0.75:55|110|180|280:0.32|0.22|0.14|0.08'];
  if(c.style==='dark_echo'){
    const pitch={low:-3.5,mid:-5,bright:-6.5}[order.voiceRange],rate=2**(pitch/12),delay=60000/order.bpm*.75;
  ...
  if(c.style==='sexy_robot'){
    const pitch={low:-1,mid:-2,bright:-3}[order.voiceRange],rate=2**(pitch/12),delay=60000/order.bpm;
    style=[`asetrate=${SR*rate}`,`aresample=${SR}`,`atempo=${1/rate}`,`aecho=0.8:0.55:${delay}|${delay*2}:0.24|0.10`,'lowpass=f=9200'];
  }
  if(c.style==='chopped_up'){
    const delay=60000/order.bpm*.25;
    style=[`asplit=2[lead][space];[space]highpass=f=240,lowpass=f=6500,aecho=0.8:0.58:${delay}|${delay*2}:0.30|0.13[echo];[lead][echo]amix=inputs=2:normalize=0`];
  }
  const duration=samples.length/SR+(c.style==='clean'?.32:c.style==='dark_echo'?60/order.bpm*2.25:c.style==='sexy_robot'?60/order.bpm*1.25:c.style==='chopped_up'?60/order.bpm*.5:0);
...
async function render(pcm,cut,order){
  ...
  if(cut.style==='sexy_robot')samples=pulse(crush(robot(samples,order.bpm)),order.bpm);
...
async function generate(order){
  ...
  const text=`${deliveries[order.delivery]||deliveries.dark} ${order.phrase}`;
  const response=await fetch(`https://api.elevenlabs.io/v1/text-to-speech/${encodeURIComponent(order.voiceId)}?output_format=pcm_48000`,{method:'POST',headers:{'xi-api-key':process.env.ELEVENLABS_API_KEY,'content-type':'application/json'},body:JSON.stringify({text,model_id:'eleven_v3'}),signal:AbortSignal.timeout(60000)});
