...
function gate(samples,bpm,groove,amount){
  ...
  const on=cycle*(amount==='half'?.5:.2),edge=Math.min(SR*.006,on*.24);
  return Float32Array.from(samples,(v,i)=>{
    const phase=i%cycle;
    if(phase>=on)return 0;
    const edgePhase=Math.min(phase,on-phase);
    const gain=edgePhase>=edge?1:.5-.5*Math.cos(Math.PI*edgePhase/edge);
    return v*gain;
  });
}
...
function filters(c,order,samples){
  ...
  const duration=samples.length/SR+(c.style==='clean'?.15:c.style==='dark_echo'?60/order.bpm*2.25:c.style==='sexy_robot'?60/order.bpm*.5:0);
...
async function generate(order){
  ...
  const text=`${deliveries[order.delivery]||deliveries.seductive} ${order.phrase}`;
