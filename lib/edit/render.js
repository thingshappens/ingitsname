...
function gate(samples,bpm,groove,amount){
  const cycle=SR*60/bpm/({straight:2,triplet:3,double:4}[groove]);
...
function robot(samples,bpm){
  // Constrained adaptation of Atelier's deterministic prepareSourceBuffer glitch.
  const beat=Math.max(128,Math.round(SR*60/bpm/4)),slice=Math.floor(beat/3);
  return Float32Array.from(samples,(v,i)=>{
    const block=Math.floor(i/beat),local=i%beat;
    if(block%4!==3)return v;
    const edge=local%slice,idx=Math.min(samples.length-1,block*beat+edge);
    return .6*v+.4*samples[idx]*Math.min(1,edge/72);
  });
}
...
function filters(c,order,samples){
  ...
  if(c.style==='sexy_robot')style=['acrusher=bits=10:mix=0.16','lowpass=f=10500'];
