...
function filters(c,order,samples){
  let style=[];
  if(c.style==='clean')style=['aecho=0.95:1:32|57|89:0.13|0.08|0.04'];
  if(c.style==='dark_echo'){
    const pitch={low:-2,mid:-4,bright:-5.5}[order.voiceRange],rate=2**(pitch/12),delay=60000/order.bpm*.75;
  ...
  const duration=samples.length/SR+(c.style==='clean'?.10:c.style==='dark_echo'?60/order.bpm*2.25:c.style==='sexy_robot'?60/order.bpm*.5:0);
