...
function gate(samples,bpm,groove,amount){
  ...
  const on=cycle*(amount==='half'?.5:.2),curve=amount==='half'?.6:.8;
  return Float32Array.from(samples,(v,i)=>{
    const phase=i%cycle;
    if(phase>=on)return 0;
    return v*Math.pow(Math.sin(Math.PI*phase/on),curve);
  });
}
...
function filters(c,order,samples){
  ...
  if(c.style==='sexy_robot'){
    ...
    style=[`asetrate=${SR*rate}`,`aresample=${SR}`,`atempo=${1/rate}`,`aecho=0.8:0.62:${delay}|${delay*1.75}|${delay*2.5}:0.22|0.11|0.05`,'lowpass=f=9800'];
  }
  if(c.style==='chopped_up'){
    const delay=60000/order.bpm*.25;
    style=[`asplit=2[lead][space];[space]highpass=f=240,lowpass=f=6500,aecho=0.8:0.55:${delay}|${delay*2}:0.16|0.06[echo];[lead][echo]amix=inputs=2:normalize=0`];
  }
...
async function render(pcm,cut,order){
  ...
  if(cut.style==='sexy_robot')samples=pulse(robot(samples,order.bpm),order.bpm);
...
module.exports={SR,gate,robot,pulse,pcmFloats,filters,render,generate,measure};
