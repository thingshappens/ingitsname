...
function filters(c,order,samples){
  let style=[];
  if(c.style==='clean'){
    const rate=2**((order.voiceProfile==='masculine'?-1.5:0)/12);
    style=[...(order.voiceProfile==='masculine'?[`asetrate=${SR*rate}`,`aresample=${SR}`,`atempo=${1/rate}`]:[]),'aecho=0.8:0.75:55|110|180|280:0.32|0.22|0.14|0.08'];
  }
  if(c.style==='dark_echo'){
    const pitch={low:-5,mid:-6.5,bright:-8}[order.voiceRange]+(order.voiceProfile==='masculine'?-1:0),rate=2**(pitch/12),delay=60000/order.bpm*.75;
  ...
  if(c.style==='sexy_robot'){
    // Direct Digital Breakdown finishing chain from Atelier: −2 pitch, Echo 24,
    // Reverb 18. The generated source keeps its articulation before this stage.
    const rate=2**((order.voiceProfile==='masculine'?-4:-2)/12),delay=60000/order.bpm;
...
async function generate(order){
  ...
  const response=await fetch(`https://api.elevenlabs.io/v1/text-to-speech/${encodeURIComponent(order.voiceId)}?output_format=pcm_48000`,{method:'POST',headers:{'xi-api-key':process.env.ELEVENLABS_API_KEY,'content-type':'application/json'},body:JSON.stringify({text,model_id:'eleven_v3',voice_settings:{stability:1,similarity_boost:.85,style:0,use_speaker_boost:true}}),signal:AbortSignal.timeout(60000)});
