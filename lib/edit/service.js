...
function settings(){
  ...
  if(!enabled||!key||!process.env.THE_EDIT_WEBHOOK_SECRET||!process.env.THE_EDIT_RUNPOD_ENDPOINT_ID||!process.env.THE_EDIT_RUNPOD_API_KEY||!process.env.STRIPE_PRICE_THE_EDIT_4||!model.voices().length)throw new Error('The Edit is not open for orders yet.');
