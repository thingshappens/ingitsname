...
function settings(){
  ...
  if(!enabled||!key||!process.env.THE_EDIT_WEBHOOK_SECRET||!process.env.ELEVENLABS_API_KEY||!process.env.STRIPE_PRICE_THE_EDIT_2||!process.env.STRIPE_PRICE_THE_EDIT_4||!model.voices().length)throw new Error('The Edit is not open for orders yet.');
...
function createService({store=storage,renderer=rendering}={}){
  async function checkout(input,requestId,token,{stripe,origin,production}){
    ...
    try{
      ...
      // Dynamic methods can omit Klarna in a Sandbox session before it has enough
      // customer context. Require it in Preview so the real Klarna flow is tested
      // without changing the production checkout's method selection.
      const paymentMethodTypes=production?undefined:['card','klarna'];
      const session=await stripe.checkout.sessions.create({mode:'payment',managed_payments:{enabled:false},allow_promotion_codes:false,...(paymentMethodTypes?{payment_method_types:paymentMethodTypes}:{}),line_items:[{price:priceId,quantity:1}],metadata:{order_id:order.id},integration_identifier:'hsc_the_edit_qmzvtrka',success_url:`${origin}/edit/?order=${order.id}#access=${token}`,cancel_url:`${origin}/edit/?cancelled=1`},{idempotencyKey:`the-edit-${order.id}`});
