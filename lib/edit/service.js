...
function settings(){
  ...
  // HSC character recordings and their delivery worker are intentionally not
  // wired here yet. Do not reopen orders by substituting an ElevenLabs voice.
  if(!enabled||!key||!process.env.THE_EDIT_WEBHOOK_SECRET||!process.env.STRIPE_PRICE_THE_EDIT_2||!model.voices().some(voice=>voice.status==='ready'))throw new Error('The Edit is not open for orders yet.');
