...
function previewSettings(request){
  ...
  // Public "Hear cut" on the live site: allowed on its own origin (THE_EDIT_ORIGIN),
  // rate-limited per visitor and site-wide in api/the-edit.js to cap GPU cost.
  if(!process.env.THE_EDIT_RUNPOD_ENDPOINT_ID||!process.env.THE_EDIT_RUNPOD_API_KEY||!model.voices().length)throw new Error('The Edit preview is not configured.');
  const origin=request?.headers?.origin;
  const host=String(request?.headers?.host||'').toLowerCase();
  let url;try{url=new URL(origin);}catch{throw new Error('Invalid preview origin');}
  let siteHost='';try{siteHost=new URL(process.env.THE_EDIT_ORIGIN).host.toLowerCase();}catch{}
  if(url.protocol!=='https:'||url.host.toLowerCase()!==host||!(host.endsWith('.vercel.app')||(siteHost&&host===siteHost)))throw new Error('Invalid preview origin');
...
function createService({store=storage,renderer=rendering}={}){
  ...
  async function paid(session,stripe){
    ...
    try{
      ...
      if(remoteFulfil()){
        // GPU box does voice + four cuts and stores the audio itself; refresh() picks up the result.
        if(order.status!=='rendering'||!order.fulfilJobId){await startFulfil(order);await store.save(order,owner);}
        return;
      }
      try{
  ...
  function remoteFulfil(){return process.env.THE_EDIT_REMOTE_FULFIL==='1';}
  async function startFulfil(order){
    const prefixes={};
    for(const c of order.cuts)prefixes[c.id]=`${store.key(order.id)}:audio:${c.id}:${crypto.randomUUID()}`;
    const slim={phrase:order.phrase,bpm:order.bpm,voiceboxProfileId:order.voiceboxProfileId,voiceboxEngine:order.voiceboxEngine,voiceboxModelSize:order.voiceboxModelSize,voiceboxLanguage:order.voiceboxLanguage,voiceboxInstruct:order.voiceboxInstruct,internalRobotCandidate:order.internalRobotCandidate};
    const cuts=order.cuts.map(({asset,metrics,...c})=>c);
    ...
    order.fulfilJobId=await voicebox.submit('fulfil',{order:slim,cuts,store:{...creds,ttl:store.AUDIO_TTL,prefixes}});
    order.fulfilPrefixes=prefixes;order.fulfilAttempts=(order.fulfilAttempts||0)+1;order.fulfilStartedAt=new Date().toISOString();
    order.status='rendering';order.lastError=null;
  }
  // Called by the order page's status poll and by the Worker's cron: applies a finished GPU job.
  async function refresh(id){
    let order=await store.get(id);
    if(!order||order.status!=='rendering'||!order.fulfilJobId)return order;
    const owner=await store.lock(id);if(!owner)return order;
    try{
      order=await store.get(id);
      if(!order||order.status!=='rendering'||!order.fulfilJobId)return order;
      const result=await voicebox.job(order.fulfilJobId);
      const out=result.output&&typeof result.output==='object'?result.output:null;
      if(['IN_QUEUE','IN_PROGRESS'].includes(result.status))return order;
      if(result.status==='COMPLETED'&&out&&Array.isArray(out.cuts)&&!out.error){
        const expiresAt=Date.now()+store.AUDIO_TTL*1000;
        for(const cut of order.cuts){
          const r=out.cuts.find(x=>x&&x.id===cut.id);
          if(!r||r.prefix!==order.fulfilPrefixes?.[cut.id]||!Number.isInteger(r.count)||!Number.isInteger(r.bytes))throw new Error('Fulfilment result does not match order');
          cut.asset={prefix:r.prefix,count:r.count,bytes:r.bytes,expiresAt};cut.metrics=r.metrics;cut.renderStatus='ready';
        }
        if(out.generation){order.voiceboxGenerationId=out.generation.id;order.voiceboxEngine=out.generation.engine||order.voiceboxEngine;order.voiceboxModelSize=out.generation.model_size||order.voiceboxModelSize;order.voiceboxSampleRate=out.generation.sample_rate||null;}
        order.status='ready';order.downloadExpiresAt=expiresAt;await store.save(order,owner);
        await store.redis().srem('hsc:edit:pending',id);
        console.info(JSON.stringify({event:'the_edit_render_ready',cutCount:order.cutCount}));
        return order;
      }
      order.lastFulfilError=String(result.error||out?.error||result.status).slice(0,500);
      console.error(JSON.stringify({event:'the_edit_fulfil_failed',attempt:order.fulfilAttempts,error:order.lastFulfilError}));
      if((order.fulfilAttempts||0)<3)await startFulfil(order);
      else{order.status='failed';order.lastError='Rendering paused. Your payment is saved; delivery will retry.';}
      await store.save(order,owner);
      return order;
    }finally{await store.unlock(id,owner);}
  }
  ...
  return {checkout,paid,authorized,refresh,startFulfil};
}
