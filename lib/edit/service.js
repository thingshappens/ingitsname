...
function settings(){
  ...
  if(!enabled||!key||!process.env.THE_EDIT_WEBHOOK_SECRET||!process.env.THE_EDIT_VOICEBOX_URL||!process.env.THE_EDIT_VOICEBOX_TOKEN||!process.env.STRIPE_PRICE_THE_EDIT_4||!model.voices().length)throw new Error('The Edit is not open for orders yet.');
