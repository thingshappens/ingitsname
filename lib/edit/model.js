...
const VERSION = 'edit-v1-candidate-12';
...
const EXCLUDED_VOICE_IDS = new Set(['N2lVS1w4EtoT3dr4eOWO']);
const DELIVERIES = {
  dark:null,
  hyped:null
};
...
function voices() {
  ...
  return Array.isArray(list) ? list.filter(v=> /^[A-Za-z0-9_-]{8,100}$/.test(v.id) && !EXCLUDED_VOICE_IDS.has(v.id) && typeof v.name==='string' && ['low','mid','bright'].includes(v.range) && v.licensed===true).map(v=>({...v,voiceProfile:MASCULINE_VOICE_IDS.has(v.id)?'masculine':'neutral'})) : [];
}
