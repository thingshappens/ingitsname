  ...
  return async function(req,res){
    ...
    try{
      ...
      if(req.headers['paddle-signature']){
        if(!env.PADDLE_WEBHOOK_SECRET)return res.status(503).end();
        paddle.verifyWebhook(raw,req.headers['paddle-signature'],env.PADDLE_WEBHOOK_SECRET);
        event=JSON.parse(raw.toString('utf8'));
        const payment=paddle.normalizePaidEvent(event);
        if(payment)await service.paid(payment,null);
