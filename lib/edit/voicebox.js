...
async function run(action,input,{env=process.env,fetchImpl=fetch}={}){
  ...
  let queued;
  try{
    // runsync waits on RunPod's side (up to 90 s) in one request. Polling /status every
    // 750 ms used ~50 requests and hit Cloudflare's per-request subrequest limit.
    const response=await fetchImpl(`${baseUrl}/runsync?wait=90000`,{method:'POST',headers:headers(apiKey),body:JSON.stringify({input:{action,...input}}),signal:AbortSignal.timeout(100000)});
  ...
  while(Date.now()<deadline){
    ...
    if(job?.status==='COMPLETED'){
      if(!job.output||typeof job.output!=='object'||typeof job.output.error==='string')throw Object.assign(new VoiceboxError('Voice rendering failed.'),{detail:String(job.output?.error||'no output').slice(0,300)});
      return job.output;
    }
    if(['FAILED','CANCELLED','TIMED_OUT'].includes(job?.status))throw Object.assign(new VoiceboxError('Voice rendering failed.'),{detail:`${job.status}: ${String(job.error||job.output?.error||'').slice(-1500)}`});
  }
