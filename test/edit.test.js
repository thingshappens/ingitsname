...
function fixture(){
  ...
  return {service,store,stripe,counts:()=>({renders,generations,sessions}),fail:()=>{fail=true;},session:id=>({id:'cs_test_valid',metadata:{order_id:id},mode:'payment',livemode:false,currency:'usd',amount_total:1200,payment_status:'paid'})};
}
...
test('four-cut purchase uses the configured 1200-cent price and fulfils every cut',async()=>{const f=fixture(),id=randomUUID();await f.service.checkout(input(),id,'a'.repeat(64),{stripe:f.stripe,origin:'https://preview.example',production:false});assert.equal((await f.store.get(id)).expectedAmount,1200);await f.service.paid(f.session(id),f.stripe);assert.deepEqual(f.counts(),{renders:4,generations:1,sessions:1});});
